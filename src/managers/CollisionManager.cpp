...
void CollisionManager::addProjectiles(){
    for(vector<Player*>::iterator it = players_vector.begin(); it != players_vector.end(); it++){
        if(*it){
            for(list<Projectile*>::iterator itP = (*it)->playerProjectiles().begin(); itP != (*it)->playerProjectiles().end(); itP++){
                if(*itP){
                    addProjectile(*itP);
                }
            }
...
void resolveCollisionProjectile(Character* c, Projectile* p){
    Vector2f cPos =  c->getPosition();
    Vector2f oPos =  p->getPosition();
    Vector2f cSize = c->getSize();
    Vector2f oSize = p->getSize();
    ...
    if (intersectX < 0.0f && intersectY < 0.0f) {
        p->changeActive();
        c->setSpeed(Vector2f(c->getSpeed().x * 0.7f, c->getSpeed().y));
        cPos.x = c->getPosition().x - c->getSpeed().x ;
        c->setPosition(cPos);
    }
}
            ...
            for(set<Projectile*>::iterator itProjectile = projectiles_set.begin(); itProjectile != projectiles_set.end(); itProjectile++){
                if(*itProjectile){
                    if(verifyCollision( (*it) , (*itProjectile) ) ){
                        resolveCollisionProjectile((*it), (*itProjectile));
                        cout<<"colidiu"<<endl;
                    }
                }
            }
