...
BackgroundElement::BackgroundElement(float x, float y, float Parallaxfactor, const string &texturePath): InitialPosition(x, y) {
...
void BackgroundElement::update(const Vector2f &camPosition) {   
    ...
    float parallaxX = InitialPosition.x + (camPosition.x - InitialPosition.x) * parallaxFactor;
    float parallaxY = InitialPosition.y + (camPosition.y - InitialPosition.y) * parallaxFactor;
